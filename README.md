...
> The steps above are the short version. The full step-by-step guide — cloud-init, DNS, nginx, SSL, MinIO, and every gotcha — lives in **[DEPLOY.md](DEPLOY.md)**.

### Backup & Recovery

Backups are created by `scripts/backup.sh` (Postgres `pg_dump` + MinIO volume tar) to `/opt/backups/cogext/` with 7-day retention, and restored with `scripts/restore.sh <timestamp>`. See [DEPLOY.md → Maintenance & Backup](DEPLOY.md#maintenance--backup) for the cron setup and download scripts.

**Bringing the app up on a fresh server from a backup** (disaster recovery, or migrating hosts):

1. Have your backup + your old `.env` values on hand (the DB password and secrets must match, or the restored data won't decrypt/authenticate). Both are captured by the [pre-teardown checklist](DEPLOY.md#server-migration-teardown--recreate).
2. Provision the new server and bring the stack up empty — [DEPLOY.md steps 1–12](DEPLOY.md#1-prepare-cloud-init).
3. Upload the backup, then run `restore.sh <timestamp>` on the server.
4. Point DNS at the new IP, re-run Certbot, and update the `DEPLOY_HOST` GitHub secret.

The full runbook is **[DEPLOY.md → Recover onto a fresh server](DEPLOY.md#recover-onto-a-fresh-server)**.
