...
import {
  ...
  real,       // single-precision float (used for book star ratings)
...
//
// NOTE on "article": retired from the create UI (link covers it for now) but
// intentionally KEPT in the enum so existing article records still validate,
// render, and edit — and so it's a one-line change to re-offer it later.
// Removing an enum value in Postgres is destructive; don't drop it.
...
// Reading-shelf status for book records (Goodreads-style). Nullable on the
// record — only book records set it.
...
export const records = pgTable("records", {
  ...
  // ---- Book-only fields (Goodreads-style reading shelf) ----
  // All nullable — only book records populate these; every other record type
  // and every pre-existing record leaves them null.

  // Star rating on a 0–5 scale, stored as a real so half/decimal ratings from
  // imported data survive (e.g. site data was /10 → normalized to /5).
  rating: real("rating"),

  // Where the book sits on the shelf: want_to_read / reading / read.
  readingStatus: readingStatusEnum("reading_status"),

  // Date the book was finished. `date` (no time) — a reading log only cares
  // about the day.
  dateRead: date("date_read"),

  // The embedding vector — 1024 floats representing the "meaning" of this
  // record's content. Generated by Voyage AI (voyage-4-lite).
  // Used by pgvector for semantic search (cosine similarity).
  //
  // Nullable because:
  //   1. Embedding happens async after record creation
  //   2. The embedding API might be down — we don't want to block saving
  ...
  //
  // Example values: "voyage-4-lite", "text-embedding-3-small"
